	...
	\begin{itemize}
		\item Clase \textbf{GestorCorreos}:
			\begin{itemize}
				\item \textit{validar}:
				\item \textit{logout}:
				\item \textit{registrar}:
				\item \textit{obtenerInfo}:
				\item \textit{cambioDomicilio}:
			\end{itemize}
			
		\item Clase \textbf{GestorEnvíos}:
			\begin{itemize}
				\item \textit{enviarDocumento}:
				\item \textit{envíarPaquete}:
				\item \textit{seguirEnvío}:
			\end{itemize}
		
		\item Clase \textbf{GestorFilatelia}:
			\begin{itemize}
				\item \textit{registrar}:
				\item \textit{buscaAsocación}:
				\item \textit{buscaInfo}:
				\item \textit{realizaPedido}:
				\item \textit{buscaSucursal}:
			\end{itemize}
		
		\item Clase \textbf{GestorOficinaVirtual}:
			\begin{itemize}
				\item \textit{doSeguimiento}:
				\item \textit{ultimosEnvíos}:
				\item \textit{justificante}:
				\item \textit{tareasPendientes}:
			\end{itemize}
			
		\item Clase \textbf{CentrosCorreos}:
			\begin{itemize}
				\item \textit{buscaAsociación}:
				\item \textit{buscaSucursal}:
			\end{itemize}
	\end{itemize}
