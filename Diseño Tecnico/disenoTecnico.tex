	...
	\begin{itemize}
		\item \textbf{Tabla Usuarios}: 
				\begin{itemize}
					...
					\item \textit{NombreUsuario}: tipo CHAR(50). UNIQUE y NOT NULL.
					\item \textit{DNI}: tipo CHAR(9). NOT NULL.
					\item \textit{Contraseña}: tipo CHAR(20).
					\item \textit{TipoUsuario}: tipo CHAR(15).
					\item \textit{Nombre}: tipo CHAR(50).
					\item \textit{Apellidos}: tipo CHAR(50).
					\item \textit{Provincia}: tipo CHAR(100).
					\item \textit{Municipio}: tipo CHAR(100).
					\item \textit{Calle}: tipo CHAR(200).
					\item \textit{Número}: tipo CHAR(3).
					\item \textit{Escalera}: tipo CHAR(2).					
					\item \textit{Letra}: tipo CHAR(1).
					\item \textit{Piso}: tipo CHAR(2).
					\item \textit{CódigoPostal}: tipo CHAR(10).
					\item \textit{Teléfono}: tipo CHAR(10).
				\end{itemize}				
		
		\item \textbf{Tabla Filatelia}:
				\begin{itemize}
					\item \textit{IDUsuario}: tipo CHAR(10). Primary Key y Foreign Key a la tabla Usuarios.
					\item \textit{NombreBanco}: tipo CHAR(20).
					\item \textit{Cuenta}: tipo CHAR(20).
					\item \textit{CantTarjetas}: tipo NUMBER.
					\item \textit{CantSellos}: tipo NUMBER.
					\item \textit{CantEtiquetas}: tipo NUMBER.
					\item \textit{CantLibros}: tipo NUMBER.
					\item \textit{CantPruebas}: tipo NUMBER.
				\end{itemize}
				
		\item \textbf{Tabla Información}:
			\begin{itemize}
				\item \textit{Código}: tipo CHAR(10). Primary Key.
				\item \textit{TipoInformación}: tipo CHAR(100). 
				\item \textit{Contenido}: tipo CHAR(800).
			\end{itemize}
		
		\item \textbf{Tabla CentroCorreos}:
			\begin{itemize}
				\item \textit{Código}: tipo CHAR(10). Primary Key.
				\item \textit{Dirección}: tipo CHAR(10).
				\item \textit{Teléfono}: tipo CHAR(10).
				\item \textit{Provincia}: tipo CHAR(10).
				\item \textit{Fax}: tipo CHAR(10).
			\end{itemize}
		\item \textbf{Tabla Sucursal}: 
				\begin{itemize}
					\item \textit{CódigoCentro}: tipo CHAR(10). Primary Key y Foreign Key a la tabla CentroCorreos.
				\end{itemize}
		\item \textbf{Tabla Asociación}: 
				\begin{itemize}
					\item \textit{CódigoCentro}: tipo CHAR(10). Primary Key y Foreign Key a la tabla CentroCorreos.
					\item \textit{Nombre}: tipo CHAR(50).
		...
		\item \textbf{Tabla Pedido}:
				\begin{itemize}
					...
					\item \textit{IDUsuario}: tipo CHAR(10). Foreign Key a la tabla Usuarios.
					\item \textit{NombrePedido}: tipo CHAR(20).
					\item \textit{NumFactura}: tipo CHAR(10). UNIQUE y NOT NULL.
					\item \textit{FechaIni}: tipo DATETIME.
					\item \textit{FechaFin}: tipo DATETIME.
					\item \textit{Estado}: tipo CHAR(15).
				\end{itemize}					
		\item \textbf{Tabla PedidoProd}: 
				\begin{itemize}
					\item \textit{IDPedido}: tipo CHAR(10). Primary Key y Foreign Key a la tabla Pedido.
					\item \textit{CódigoProd}: tipo CHAR(10). Primary Key y Foreign Key a la tabla Productos.
					\item \textit{ContenidoAdicional}: tipo CHAR(200).
					\item \textit{Cantidad}: tipo NUMBER.
				\end{itemize}
		...
		\item \textbf{Tabla Productos}:
				\begin{itemize}
					\item \textit{Código}: tipo CHAR(10). Primary Key.
					\item \textit{Nombre}: tipo CHAR(50).
					\item \textit{Tipo}: tipo CHAR(20).
					\item \textit{Descripción}: tipo CHAR(400).
					\item \textit{Precio}: tipo CHAR(10).
				\end{itemize}
				
		\item \textbf{Tabla Colecciones}: 
				\begin{itemize}
					\item \textit{Código}: tipo CHAR(10). Primary Key.
					\item \textit{Nombre}: tipo CHAR(20).
					\item \textit{Descripción}: tipo CHAR(200).
				\end{itemize}

		\item \textbf{Tabla ProductoColección}: 
				\begin{itemize}
					\item \textit{CódigoProducto}: tipo CHAR(10). Primary Key y Foreign Key a la tabla Productos.
					\item \textit{CódigoColección}: tipo CHAR(10). Primary Key y Foreign Key a la tabla Colecciones.
				\end{itemize}
				
		\item \textbf{Tabla Envío}:
				\begin{itemize}
					\item \textit{IDEnvio}: tipo CHAR(10). Primary Key.			
					\item \textit{IDUsuario}: tipo CHAR(10). Unique  y Foreign Key a la tabla Usuarios.
					\item \textit{NombreEnvio}: tipo CHAR(50).
					\item \textit{NumFactura}: tipo NUMBER. UNIQUE y NOT NULL.
					\item \textit{FechaIni}: tipo DATETIME.
					\item \textit{FechaFin}: tipo DATETIME.
					\item \textit{Estado}: tipo CHAR(15).
					\item \textit{NombreDest}: tipo CHAR(50).
					\item \textit{ApellidosDest}: tipo CHAR(20).
					\item \textit{DNIDest}: tipo CHAR(9).
					\item \textit{DirecciónDest}: tipo CHAR(30).
					\item \textit{NúmeroDest}: tipo CHAR(3).
					\item \textit{PisoDest}: tipo CHAR(2).
					\item \textit{LetraDest}: tipo CHAR(1).
					\item \textit{EscaleraDest}: tipo CHAR(2).
					\item \textit{TeléfonoDest}: tipo CHAR(10).					
					\item \textit{NombreRem}: tipo CHAR(50).
					\item \textit{ApellidosRem}: tipo CHAR(20).
					\item \textit{DNIRem}: tipo CHAR(9).
					\item \textit{DirecciónRem}: tipo CHAR(30).
					\item \textit{NúmeroRem}: tipo CHAR(3).
					\item \textit{PisoRem}: tipo CHAR(2).
					\item \textit{LetraRem}: tipo CHAR(1).
					\item \textit{EscaleraRem}: tipo CHAR(2).
					\item \textit{TeléfonoRem}: tipo CHAR(10).
				\end{itemize}
		
		\item \textbf{Tabla EnvíoProd}: 
				\begin{itemize}
					\item \textit{IDEnvío}: tipo CHAR(10). Primary Key y Foreign Key a la tabla Envío.
					\item \textit{CódigoProd}: tipo CHAR(10). Foreign Key a la tabla Productos.
					\item \textit{ContenidoAdicional}: tipo CHAR(200).
				\end{itemize}
	\end{itemize}
...
\subsection{DSI 6.2: Especificación de los Caminos de Acceso a los Datos}
	...
	\begin{itemize}
		...
		\item En la tabla PEDIDO habrá un índice compuesto por \textit{IDPedido} y \textit{NumFactura} para buscar de forma más rápida un usuario.
		\item ...
	\end{itemize}
